...
// is trapped, Escape closes, focus returns to the opener. Errors are shown INSIDE the modal (the
// brief requires in-modal errors with a retry path). A click on the backdrop does not close it, so
// a half-entered transaction is never lost to a stray click.
...
export function openModal({ title, body, actions = [], onClose = () => {}, wide = false }) {
  ...
  const error = el("div", { class: "modal__error", role: "alert", hidden: true });
  const closeBtn = el("button", { type: "button", class: "modal__close", "aria-label": "Close", text: "×" });
  const dialog = el("div", { class: "modal", role: "dialog", "aria-modal": "true", "aria-labelledby": titleId, vars: wide ? { "--modal-width": "56rem" } : {} }, [
  ...
  function close() {
    if (closed) return;
    closed = true;
    backdrop.remove ? backdrop.remove() : backdrop.parentNode && backdrop.parentNode.removeChild(backdrop);
  ...
  function focusables() {
    return Array.from(dialog.querySelectorAll(FOCUSABLE)).filter((n) => !n.disabled && n.getAttribute("tabindex") !== "-1" && !n.hidden);
  }
