...
export function categoryIndex(state) {
  ...
  const personal = (state.preferences && state.preferences.effective && state.preferences.effective.categoryColors) || {};
  const map = new Map();
  for (const c of (data && data.categories) || []) map.set(c.id, { ...c, shownColor: personal[c.id] || c.color });
  return map;
}
