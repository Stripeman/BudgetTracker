...
- **Milestone reviews of `be22642`** (independent, read-only).
  - The security review found no way to read another workspace or another member's private records (PARTIAL). Its nine findings are all fixed with regression tests S1–S9.
  - The financial review verified the core money arithmetic (PARTIAL). Its twelve defects are fixed with regression tests F1–F12. The allocation gaps for shared expenses are deferred to BT-009.
  - See `docs/REQUIREMENTS.md` BT-001-04 and BT-006-02.
- **Azure.** BudgetTracker has its own resource group, Standard Static Web App, monitoring and preview-only storage, all separate from TaskTracker. Production is not provisioned or deployed. Deployment residual risks (a single SWA deployment token, agents using admin credentials, connection strings, backup-key escrow) are recorded in `docs/DEPLOYMENT.md`.
