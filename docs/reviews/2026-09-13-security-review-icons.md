...
| SEC-I4 | Low | The catalogue document had no size cap and its audit/history grow; every signed-in user downloads it on workspace switch | **Mostly fixed**: writes are refused beyond 6 MB (never trimmed); `GET /api/icons` returns a `catalogEtag`, and a client that sends back the version it holds receives `catalog: null` instead of the catalogue again, so a workspace switch no longer re-downloads it (tested). Open: partition the catalogue's audit and history (ADR-003) |
...
| SEC-I7 | Info | Personal category icons (like personal colours) are replaced without a change history | **Resolved by Terry (2026-09-13)**: no — personal display preferences need no change history |
