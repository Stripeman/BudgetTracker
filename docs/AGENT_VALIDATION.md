...
## Claude takeover — 2026-09-13

- **Generation.** The generator now emits nine roles in both Codex TOML and Claude Markdown, and mirrors skills to `.claude/skills/`. The seven original TOMLs regenerated byte-identical; `code-archaeologist.toml` then changed deliberately (the TaskTracker checkout-selection rule). All nine TOMLs parse with Python tomllib: eight read-only, one workspace-write.
- **Executed delegations.** Two read-only general-purpose Claude subagents ran. One was an independent security and financial review of `lib/foundation.cjs` (verdict PARTIAL, findings recorded in `docs/FOUNDATION_DESIGN.md`). The other was a map of TaskTracker's frontend seams. Both were prompted with role instructions. They are not proof that the generated `.claude/agents` files load natively, because this session started before those files existed.
- **Evidence correction.** The earlier archaeology read the stale `Z:` checkout, so its editor and theme "mismatches" were wrong. See `docs/TASKTRACKER_REUSE.md`.
