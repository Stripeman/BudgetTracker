...
| Workspace GET `settingsHistory` (UX review of `eefd115`, decision 6): every member, not only owners and managers | Not sent (older API); owners/managers still get the full `history`/`lifecycle` |
| `store.loadWorkspace`/`store.mutateWorkspace` refuse an archived (deleted) workspace for every member except an owner on the delete/restore route itself (`{ archived: true }`); `workspace-model.listed()` lists a deleted workspace only for its owners | An archived workspace answered normally for every member and appeared in everyone's lists (the status change alone was not enough to actually take it away) |
...
- **Workspace settings, UX/security review fixes and FIN-1 (`eefd115`, then `fix/workspace-settings-ux` from `53cf181`).** The workspace and group settings share one client card (`app/js/ui/settingsform.js createSettingsForm`), so `app/js/ui/views/workspace.js` and `app/js/ui/views/group.js` render and behave identically; the server-side model changes below apply to both `api/_shared/workspace-settings.js` and `api/_shared/group-settings.js` unless noted.
  - **Error details.** Every refusal names the setting: an unknown key or an invalid value is `400 invalid_setting` with `details: { setting: <key> }` and a message such as `“<label>”: enter a whole number from <min> to <max>.` (an integer with no options), `“<label>”: choose one of the options shown.` (choice, boolean or set), or a set's own `requiresMessage` when one option needs another ticked (for example "restore-deleted" needs "merge"); a key above the caller's role is `403` with `details: { setting: <key> }` and `Only <who> can change “<label>”.`. The client marks the control `aria-invalid`/`aria-errormessage`, opens its group and focuses it.
  - **`settingsHistory` (decision 6).** `GET /api/workspaces?id=` returns `settingsHistory` — every `history` entry whose changes touch `settings.*`, with `at`, `by` (or "Former member"), the filtered `changes` and `reason` — to every member, not only owners and managers; the full `history`/`lifecycle` (name changes, archive/restore) stay owners-and-managers-only. Someone who cannot change a setting reads its value and this history as a label–value list (`dl`), with only owner-only settings marked "Owners only" and who changes settings said once in the card's intro, not per setting.
  - **L-1: tolerant backup integrity.** `workspace-settings.js problem(doc)` accepts any well-typed scalar (`null`, boolean, string or a finite number) for a setting this server version does not recognise, and any array of well-typed scalars for a `set` — the strict "every key must be one we know" check refused a workspace with a setting a newer version added, which a restore or a rolling deploy can legitimately produce; it still refuses the wrong shape (an object, a function, `NaN`, etc.).
  - **L-2: Shared Expenses re-checked inside every group write.** `api/group/handler.js` reads the site once per request (`gateSite`, a `WeakMap` keyed on `ctx`) and every one of its seven `store.mutateWorkspace` calls goes through `mutateGroup(ctx, wsId, fn, options)`, which re-runs `workspaceSettings.assertSharedExpenses(doc, site)` against the freshly read document inside the ETag-guarded write, not only once at the route's entry — closing the gap where a workspace could be switched off between the entry check and the write landing.
  - **M-1 and L-3: both sides of a transfer need the change right.** A transfer bill's `mayChangeBill(doc, principal, account, bill, capability, now)` requires `canChangeRecord` on both the source and destination accounts, so `canEdit`/`canDelete` (and skip/unskip/pause/resume, PATCH and DELETE) are offered only when both hold; a transfer entry's `canEdit`/`canDelete` likewise require every other leg's account (built from a `transferId` → legs map) to pass `canChangeRecord`, not only the account being viewed. Before this, "any entry" on a shared account let a member change a transfer bill or entry that also touched an account they could not otherwise reach.
  - **I-2: a member's recovery-point cap follows their restore allowance.** `reserveRecoveryPoint(ctx, wsId, subject, perPerson)` in `api/backups/handler.js` defaults to 6 a day, but `api/restore/handler.js` calls it with `2 × min(memberRestoresPerDay, 3)` for a non-manager, so the pre-restore safety copy a member's own restore causes is bounded by the same setting that bounds the restores themselves, never above the fixed ceiling.
  - **Accepted, documented-only findings.** I-1: a settings PATCH body of only `__proto__`/`constructor` keys is a 200 no-op (`fields.onlyKeys`/`parseChanges` drop them; nothing is read from or written to the prototype). I-3: while Shared Expenses is off, an owner's restore preview and restore history still count shared expenses and settlements already in the backup or workspace — accepted, since nothing is exposed to anyone but an owner or manager and nothing new is created while it is off.
  - **FIN-1 (financial recheck of `53cf181`).** Creating a budget (`POST /api/budgets`) whose `startDate` is before its `budgetPeriod`'s current period start (`budgeting.periodFor(...).start` for today) is refused `409 backdate_off` under `budgetBackdating: 'never'` (starting exactly at the current period's start stays allowed); under `'confirm'` (the default) it needs `confirmBackdate: true` in the body or is refused `409 backdate_unconfirmed` naming the current period's start. Changing an existing budget's dates already followed this setting (BT-011-12); creating one did not, until now — a behaviour change: a new budget starting in a finished period now needs confirmation by default, where it was silently allowed before.
- **Owners delete their own workspace; no site-administrator delete (Terry, 2026-09-14; `fix/workspace-settings-ux`).** Shown as "Delete workspace", underneath it stays the existing recoverable archive (`DELETE /api/workspaces?id=`, owner only; `POST ?id=&action=restore`, "Bring back" in My settings) — nothing is ever physically deleted (BT-001-05). Fixed so a deleted workspace is actually taken away, not only marked: `store.loadWorkspace`/`store.mutateWorkspace` take an `archived` option (default false) and refuse an archived workspace with `404 "This workspace is not available."` for every member — owners included — unless the caller is an owner on the delete route (idempotent) or the restore route; outsiders and site administrators still get the ordinary "Unknown workspace." with no existence oracle either way. `workspace-model.listed(doc, member)` decides whether a workspace appears in `/api/me` and `/api/workspaces`: an active member's workspace, or an archived one for its owners only (so it leaves members' lists and reappears once brought back). Invitation preview and accept refuse an archived workspace. The active-workspace-creation limit (`assertCanCreateWorkspace`) and a refused "bring back" both speak of deleting, not archiving. Client: `store.actions.deleteWorkspace`/`restoreWorkspace` mirror `selectWorkspace`'s synchronous reset and generation guard — a delete of the open workspace switches to another open workspace (or onboarding) in the same commit as the refreshed list, so nothing from the deleted one can render again; the header's workspace picker, the onboarding check and the "Default workspace" preference all filter to workspaces that are not archived. Never built here: a site-administrator force-delete for a workspace whose owner is unreachable — a decision pending with Terry.
...
- **Linking, removing and reviewing (financial recheck of 41494d1, FA-1 to FA-3; decision 2026-09-14).**
  - **Backdated cash on a first link (FA-1).** Starting to record on an account for the first time in a currency (no active link yet) can silently backdate confirmed cash the person has not yet seen — an advance, reimbursement or repayment already waiting — changing the account's balance the instant the link is made. `POST /api/group?action=ledger` (and creating or confirming an expense or payment with `ledger` in the same request) now refuses this with `409 confirm_backdated`, describing the count and total, unless `confirmBackdated: true` is sent (mirroring `confirmShare` on account sharing) or the request already names the one record it concerns (its own amount is the caller's current action, not a surprise). Non-cash payable/expense-share entries, entries already recorded anywhere (including through an increment-1 per-record link), and any relink once a currency is already linked, are never blocked. The client shows the server's own message in a second dialog with Link anyway or Cancel, on every dialog that can start a first link.
  - **A removed account that is still linked (FA-2).** The account view returns `groupLedgerLinked` (true only while an active currency-wide link points at this account) to the account's own owner only — nobody else may know which account, or whether one, is linked (S2). Removing such an account still works, but the Remove dialog adds "This account is linked in Shared expenses. If you record your part there again, it will be recorded on a different account."
  - **A correction the viewer cannot see (FA-3).** Correcting or voiding a shared expense stays allowed even while another member's linked account cannot be written to (for example it is closed): privacy comes before blocking, and the corrector must never learn anything about someone else's account. That member's part is left needing review; the Dashboard's Needs attention section now also says "Shared expenses needs your attention" (linking to Shared expenses) whenever any of the viewer's own linked currencies has entries needing review, not only the passive banner on the Shared expenses page itself.
  - **Stopping recording (not yet built in the UI).** `POST /api/group?action=ledger { currency, accountId: null }` already reverses every entry it made in that currency, cash-moving ones included, and is API-only today. When a "stop recording" control is added to the client, its confirmation text must say plainly that it reverses real cash movements there too, not only shares — the same care as the FA-1 warning, in the other direction.
