// BT-013-15: Financial Overview (`acru-overview`) — every required page plus its two new extra pages
// (Merchants, Debt/loan detail) now carry the SAME "restrained header, main+side grid, segmented-bar
// breakdown" identity its own Dashboard already established (BT-013-10), not a generic shared
// template. Real-browser evidence, never a generated image or a written description.
export const name = "acrupages";
export const title = "BT-013-15: Financial Overview — every page reference-matched, plus Merchants and Debt/loan detail";
export const needsBrowser = true;

const CONCEPT_ID = "acru-overview";
...
const PAGES = [
  { id: "transactions", label: "Transactions", marker: /Transaction activity/i },
  { id: "bills", label: "Bills", marker: /Payment timeline/i },
  { id: "budget", label: "Budget", marker: /Budget health/i },
...
async function openPage(session, pageLabel) {
  ...
  await session.waitFor("!!document.querySelector('.gpreview-pane .gframe')", { what: "the Financial Overview preview frame" });
...
export async function run(h, t) {
  const { dave } = await h.browsers(["dave"], { prefix: "acrupages-desktop-", width: 1440, height: 1000 });
  ...
  const { dave: mobile } = await h.browsers(["dave"], { prefix: "acrupages-mobile-", width: 390, height: 844 });
  for (const p of PAGES) {
  ...
  await mobile.shot("accounts-mobile");
