// BT-013-15: Budget Workspace (`finexa-budget`) — every required page plus its two new extra pages
// (Merchants, Debt/loan detail) now carry the SAME "subhead + primary action, two-column top, varied-
// chart card grid" identity its own Budget anchor already established (BT-013-10), not a generic
// shared template. Real-browser evidence, never a generated image or a written description.
export const name = "finexapages";
export const title = "BT-013-15: Budget Workspace — every page reference-matched, plus Merchants and Debt/loan detail";
export const needsBrowser = true;

const CONCEPT_ID = "finexa-budget";
// Card headings render visually upper-cased (CSS text-transform); `.innerText` reflects that, so
// every marker is matched case-insensitively rather than assuming a particular DOM casing.
const PAGES = [
  { id: "dashboard", label: "Dashboard", marker: /Net position trend/i },
  { id: "transactions", label: "Transactions", marker: /Daily net movement/i },
  { id: "bills", label: "Bills", marker: /Payment timeline/i },
  { id: "budget", label: "Budget", marker: /Budget Utilization|Budget utilization/i },
  { id: "accounts", label: "Accounts", marker: /Net position/i },
  { id: "merchants", label: "Merchants", marker: /Spend by merchant/i },
  { id: "debt", label: "Debt detail", marker: /Paid off/i },
...
async function openPage(session, pageLabel) {
  ...
  await session.waitFor("!!document.querySelector('.gpreview-pane .gframe')", { what: "the Budget Workspace preview frame" });
...
export async function run(h, t) {
  const { dave } = await h.browsers(["dave"], { prefix: "finexapages-desktop-", width: 1440, height: 1000 });
  ...
  t.check("selecting an event shows a genuinely different DETAIL view: real per-event balances and settlement", {
  ...
  const { dave: mobile } = await h.browsers(["dave"], { prefix: "finexapages-mobile-", width: 390, height: 844 });
  for (const p of PAGES) {
  ...
  await mobile.shot("budget-mobile");
